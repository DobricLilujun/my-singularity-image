...
if [ ! -d "$VENV" ];then
    ...
    singularity exec --nv jupyter.sif python3 -m virtualenv $VENV --system-site-packages
    singularity run --nv jupyter.sif $VENV "python3 -m pip install --upgrade pip" 
    # singularity run --nv jupyter_parallel.sif $VENV "python3 -m pip install <your_packages>"
    singularity run --nv jupyte.sif $VENV "python3 -m ipykernel install --sys-prefix --name HPC_SCHOOL_ENV_CUDA --display-name HPC_SCHOOL_ENV_CUDA"

fi

singularity run --nv jupyter.sif $VENV "ipython profile create --parallel ${profile}"

# Enable IPython clusters tab in Jupyter notebook
singularity run --nv jupyter.sif $VENV "jupyter nbextension enable --py ipyparallel"

singularity run --nv jupyter.sif $VENV "jupyter notebook --ip ${IP_ADDRESS} --no-browser --port 8889" &
sleep 5s
singularity run --nv jupyter.sif $VENV "jupyter notebook list"
singularity run --nv jupyter.sif $VENV "jupyter --paths"
singularity run --nv jupyter.sif $VENV "jupyter kernelspec list"
