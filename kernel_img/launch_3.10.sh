...
singularity instance start jupyter_image_v1.0.0.sif jupyter

if [ ! -d "$VENV" ];then
    singularity exec instance://jupyter python3.10 -m venv $VENV --system-site-packages
    # singularity run instance://jupyter $VENV "python3 -m pip install <your_packages>"
    singularity run instance://jupyter $VENV "python3 -m ipykernel install --sys-prefix --name my_kernel --display-name my_kernel"
